...
bool DeckLinkDevice::startCapture(BMDDisplayMode displayMode, IDeckLinkScreenPreviewCallback* screenPreviewCallback, bool applyDetectedInputMode, BMDPixelFormat requestedPixelFormat)
{
	BMDVideoInputFlags		videoInputFlags = bmdVideoInputFlagDefault;

	// An explicit pixel format is a fixed capture request. In that case the DeckLink
	// input must not be restarted by the automatic format detector.
	m_applyDetectedInputMode = applyDetectedInputMode && (requestedPixelFormat == bmdFormatUnspecified);

	// Enable input video mode detection only for automatic capture format selection.
	if (m_applyDetectedInputMode && m_supportsFormatDetection)
		videoInputFlags |= bmdVideoInputEnableFormatDetection;

	BMDPixelFormat pixelFormat = requestedPixelFormat;

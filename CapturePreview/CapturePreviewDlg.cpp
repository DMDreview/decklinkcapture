...
void CCapturePreviewDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CAPTURE_PIXEL_FORMAT_COMBO, m_capturePixelFormatCombo);
	DDX_Control(pDX, IDC_RECORDING_FORMAT_COMBO, m_recordingFormatCombo);
	DDX_Control(pDX, IDC_OUTPUT_PATH_EDIT, m_outputPathEdit);
	DDX_Control(pDX, IDC_BROWSE_OUTPUT_BUTTON, m_browseOutputButton);
	DDX_Control(pDX, IDC_RECORD_BUTTON, m_recordButton);
	DDX_Control(pDX, IDC_RECORDING_STATUS_STATIC, m_recordingStatus);
}
...
BEGIN_MESSAGE_MAP(CCapturePreviewDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BROWSE_OUTPUT_BUTTON, &CCapturePreviewDlg::OnBrowseOutput)
	ON_CBN_SELCHANGE(IDC_RECORDING_FORMAT_COMBO, &CCapturePreviewDlg::OnRecordingFormatChanged)
	ON_BN_CLICKED(IDC_RECORD_BUTTON, &CCapturePreviewDlg::OnRecordBnClicked)
...
{
	const bool autoDetect = (m_applyDetectedInputModeCheckbox.GetCheck() == BST_CHECKED);
	m_modeListCombo.EnableWindow(m_selectedDevice && !autoDetect ? TRUE : FALSE);
	m_capturePixelFormatCombo.EnableWindow(!autoDetect ? TRUE : FALSE);
	if (autoDetect)
		m_capturePixelFormatCombo.SetCurSel(0);
	RefreshVideoModeList();
}
...
void CCapturePreviewDlg::RefreshVideoModeList()
{
	...
	m_selectedDevice->queryDisplayModes([this](CComPtr<IDeckLinkDisplayMode>& deckLinkDisplayMode)
	{
		...
		BMDPixelFormat supportPixelFormat = GetSelectedCapturePixelFormat();
		if (supportPixelFormat == bmdFormatUnspecified)
			supportPixelFormat = bmdFormat8BitYUV;

		if ((m_selectedDevice->getDeckLinkInput()->DoesSupportVideoMode(m_selectedInputConnection, deckLinkDisplayMode->GetDisplayMode(), supportPixelFormat,
...
void CCapturePreviewDlg::StartCapture()
{
	...
	if (m_selectedDevice)
	{
		BMDPixelFormat requestedPixelFormat = GetSelectedCapturePixelFormat();
		if (applyDetectedInputMode)
			requestedPixelFormat = bmdFormatUnspecified;

		if (m_selectedDevice->startCapture((BMDDisplayMode)m_modeListCombo.GetItemData(selectedVideoFormatIndex), m_previewWindow, applyDetectedInputMode, requestedPixelFormat))
		{
			// Update UI
...
void CCapturePreviewDlg::EnableInterface(bool enabled)
{
	...
	if (enabled)
	{
		...
		else
		{
			...
			m_capturePixelFormatCombo.EnableWindow(TRUE);
		}
	}
	else
	{
		m_applyDetectedInputModeCheckbox.EnableWindow(FALSE);
		m_modeListCombo.EnableWindow(FALSE);
		m_capturePixelFormatCombo.EnableWindow(FALSE);
	}
	RefreshRecordingInterface();
}
...
void CCapturePreviewDlg::VideoFrameArrived(CComPtr<IDeckLinkVideoInputFrame>& videoFrame)
{
	...
	if (m_recorder && m_recorder->isRecording())
		m_recorder->pushFrame(videoFrame);
...
BOOL	CCapturePreviewDlg::OnInitDialog()
{
	...
	m_recorder = std::make_unique<FrameRecorder>();

	// Capture pixel formats exposed by the DeckLink SDK. The first item is automatic detection.
	m_capturePixelFormatCombo.AddString(_T("Auto (detected)"));
	m_capturePixelFormatCombo.SetItemData(0, bmdFormatUnspecified);
	m_capturePixelFormatCombo.AddString(_T("8-bit YUV 4:2:2 (bmdFormat8BitYUV)"));
	m_capturePixelFormatCombo.SetItemData(1, bmdFormat8BitYUV);
	m_capturePixelFormatCombo.AddString(_T("10-bit YUV 4:2:2 (v210)"));
	m_capturePixelFormatCombo.SetItemData(2, bmdFormat10BitYUV);
	m_capturePixelFormatCombo.AddString(_T("8-bit ARGB"));
	m_capturePixelFormatCombo.SetItemData(3, bmdFormat8BitARGB);
	m_capturePixelFormatCombo.AddString(_T("8-bit BGRA"));
	m_capturePixelFormatCombo.SetItemData(4, bmdFormat8BitBGRA);
	m_capturePixelFormatCombo.AddString(_T("10-bit RGB (r210)"));
	m_capturePixelFormatCombo.SetItemData(5, bmdFormat10BitRGB);
	m_capturePixelFormatCombo.AddString(_T("12-bit RGB (R12B)"));
	m_capturePixelFormatCombo.SetItemData(6, bmdFormat12BitRGB);
	m_capturePixelFormatCombo.AddString(_T("12-bit RGB LE (R12L)"));
	m_capturePixelFormatCombo.SetItemData(7, bmdFormat12BitRGBLE);
	m_capturePixelFormatCombo.AddString(_T("10-bit RGB LE (R10l)"));
	m_capturePixelFormatCombo.SetItemData(8, bmdFormat10BitRGBXLE);
	m_capturePixelFormatCombo.AddString(_T("10-bit RGB BE (R10b)"));
	m_capturePixelFormatCombo.SetItemData(9, bmdFormat10BitRGBX);
	m_capturePixelFormatCombo.AddString(_T("10-bit YUVA 4:2:2 (Ay10)"));
	m_capturePixelFormatCombo.SetItemData(10, bmdFormat10BitYUVA);
	m_capturePixelFormatCombo.AddString(_T("H.265 compressed (bmdFormatH265)"));
	m_capturePixelFormatCombo.SetItemData(11, bmdFormatH265);
	m_capturePixelFormatCombo.AddString(_T("DNxHR compressed (bmdFormatDNxHR)"));
	m_capturePixelFormatCombo.SetItemData(12, bmdFormatDNxHR);
	m_capturePixelFormatCombo.SetCurSel(0);

	m_recordingFormatCombo.AddString(_T("Native SDK buffer (exact raw bytes)"));
	m_recordingFormatCombo.SetItemData(0, (DWORD_PTR)RecordingMode::NativeRaw);
	m_recordingFormatCombo.AddString(_T("FFV1 lossless (planar, unpack only; no RGB color conversion)"));
	m_recordingFormatCombo.SetItemData(1, (DWORD_PTR)RecordingMode::FFV1Lossless);
	m_recordingFormatCombo.AddString(_T("FFV1 Packed RGB8 transport -> YCbCr 4:2:2 12-bit (BT.2020 PQ)"));
	m_recordingFormatCombo.SetItemData(2, (DWORD_PTR)RecordingMode::FFV1PackedRGB8ToYCbCr422P12);
	m_recordingFormatCombo.SetCurSel(1);
	m_outputPathEdit.SetWindowText(_T("capture.mkv"));
	m_recordingStatus.SetWindowText(_T("Idle"));
...
BMDPixelFormat CCapturePreviewDlg::GetSelectedCapturePixelFormat()
{
    int i = m_capturePixelFormatCombo.GetCurSel();
    if (i < 0) return bmdFormatUnspecified;
    return (BMDPixelFormat)m_capturePixelFormatCombo.GetItemData(i);
}

RecordingMode CCapturePreviewDlg::GetSelectedRecordingMode()
{
    int i = m_recordingFormatCombo.GetCurSel();
    if (i < 0) return RecordingMode::FFV1Lossless;
    return (RecordingMode)m_recordingFormatCombo.GetItemData(i);
}

void CCapturePreviewDlg::RefreshRecordingInterface()
{
    const bool capturing = m_selectedDevice && m_selectedDevice->isCapturing();
    const bool recording = m_recorder && m_recorder->isRecording();
    m_recordButton.EnableWindow(capturing ? TRUE : FALSE);
    m_browseOutputButton.EnableWindow(recording ? FALSE : TRUE);
    m_recordingFormatCombo.EnableWindow(recording ? FALSE : TRUE);
    m_outputPathEdit.EnableWindow(recording ? FALSE : TRUE);
    m_capturePixelFormatCombo.EnableWindow((!capturing && m_applyDetectedInputModeCheckbox.GetCheck() == BST_UNCHECKED) ? TRUE : FALSE);
    m_recordButton.SetWindowText(recording ? _T("Stop recording") : _T("Start recording"));
}

void CCapturePreviewDlg::OnBrowseOutput()
{
    CFileDialog dlg(FALSE, _T("mkv"), nullptr, OFN_OVERWRITEPROMPT | OFN_PATHMUSTEXIST,
                    _T("Matroska (*.mkv)|*.mkv|Raw capture (*.raw)|*.raw|All files (*.*)|*.*||"), this);
    if (dlg.DoModal() == IDOK)
        m_outputPathEdit.SetWindowText(dlg.GetPathName());
}

void CCapturePreviewDlg::OnRecordingFormatChanged()
{
    if (GetSelectedRecordingMode() == RecordingMode::FFV1PackedRGB8ToYCbCr422P12 && m_selectedDevice && !m_selectedDevice->isCapturing())
    {
        // This mode expects the special packed RGB8 transport. The incoming BGRA8 bytes
        // are unpacked directly into 12-bit Y/Cb/Cr; there is no RGB->YCbCr matrix conversion.
        m_applyDetectedInputModeCheckbox.SetCheck(BST_UNCHECKED);
        for (int i=0;i<m_capturePixelFormatCombo.GetCount();++i)
            if ((BMDPixelFormat)m_capturePixelFormatCombo.GetItemData(i) == bmdFormat8BitBGRA) { m_capturePixelFormatCombo.SetCurSel(i); break; }
        m_capturePixelFormatCombo.EnableWindow(TRUE);
        RefreshVideoModeList();
        m_outputPathEdit.SetWindowText(_T("capture_yuv422p12_packed.mkv"));
    }
    else if (GetSelectedRecordingMode() == RecordingMode::NativeRaw)
    {
        m_outputPathEdit.SetWindowText(_T("capture.raw"));
    }
    else
    {
        m_outputPathEdit.SetWindowText(_T("capture.mkv"));
...
void CCapturePreviewDlg::StartRecording()
{
    if (!m_selectedDevice || !m_selectedDevice->isCapturing() || !m_recorder)
        return;

    CString path;
    m_outputPathEdit.GetWindowText(path);
    if (path.IsEmpty())
    {
        AfxMessageBox(_T("Select an output file."));
        return;
    }

    int modeIndex = m_modeListCombo.GetCurSel();
    if (modeIndex < 0)
    {
        AfxMessageBox(_T("No video mode is selected."));
        return;
    }

    CComPtr<IDeckLinkDisplayMode> displayMode;
    if (m_selectedDevice->getDeckLinkInput()->GetDisplayMode((BMDDisplayMode)m_modeListCombo.GetItemData(modeIndex), &displayMode) != S_OK || !displayMode)
    {
        AfxMessageBox(_T("Unable to query the selected display mode."));
        return;
    }

    BMDTimeValue duration = 0; BMDTimeScale scale = 0;
    if (displayMode->GetFrameRate(&duration, &scale) != S_OK || duration <= 0 || scale <= 0)
    {
        AfxMessageBox(_T("Unable to determine frame rate."));
        return;
    }

    double fps = double(scale) / double(duration);
    std::wstring wpath(path.GetString());
    const RecordingMode recMode = GetSelectedRecordingMode();
    auto hasExt = [&](const wchar_t* ext) {
        if (wpath.size() < wcslen(ext)) return false;
        std::wstring tail = wpath.substr(wpath.size() - wcslen(ext));
        for (auto& c : tail) c = (wchar_t)towlower(c);
        std::wstring wanted(ext);
        for (auto& c : wanted) c = (wchar_t)towlower(c);
        return tail == wanted;
    };
    if (recMode == RecordingMode::NativeRaw) { if (!hasExt(L".raw")) wpath += L".raw"; }
    else { if (!hasExt(L".mkv")) wpath += L".mkv"; }
    if (!m_recorder->start(recMode, wpath, fps, GetSelectedCapturePixelFormat()))
    {
        CString msg(m_recorder->lastError().c_str());
        AfxMessageBox(msg);
        return;
    }
    m_recordingStatus.SetWindowText(_T("Recording"));
    RefreshRecordingInterface();
}

void CCapturePreviewDlg::StopRecording()
{
    ...
    if (wasRecording)
    {
        std::wstring err = m_recorder->lastError();
        if (!err.empty())
            m_recordingStatus.SetWindowText(CString(err.c_str()));
        else
            m_recordingStatus.SetWindowText(_T("Stopped"));
    }
    RefreshRecordingInterface();
}

void CCapturePreviewDlg::OnRecordBnClicked()
{
    if (m_recorder && m_recorder->isRecording())
        StopRecording();
    else

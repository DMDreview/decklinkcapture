...
HRESULT			PreviewWindow::DrawFrame(IDeckLinkVideoFrame* theFrame)
{
	...
	// Keep the displayed video at a fixed 16:9 aspect ratio. The preview helper
	// paints into the current viewport, so use a centered 16:9 viewport and leave
	// the unused area as black bars instead of stretching the picture.
	const int viewW = rectWindow.right - rectWindow.left;
	const int viewH = rectWindow.bottom - rectWindow.top;
	if (viewW > 0 && viewH > 0)
	{
		int vpW = viewW;
		int vpH = (viewW * 9) / 16;
		...
		const int vpX = (viewW - vpW) / 2;
		const int vpY = (viewH - vpH) / 2;
